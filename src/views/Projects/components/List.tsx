...
const ProjectList = () => {
  ...
  const {data, getData: getProject, isLoading, openEditModal: edit, setOpenEditModal: setEdit} = projects || {}
  ...
  return (
    <Box>
        <Typography variant="h6" component="h1" fontWeight='bold'> Project List </Typography>

        <Box my={3}>
          <ExportTableButton disabled={isLoading} data={exported} filename="Projects" text="Export Projects" />
          <TableWrapper>
            <TableHeadWrapper>
            ...
            <TableBodyWrapper>
              {
                ...
                :
                data.length ?
                  data?.map((item: any, index: number) => <StyledTableRow key={item.id}>
                    ...
                    <StyledTableCell>
                      <Box display='flex' gap="10px">
                        <ViewButton title="View" onClick={() => handleView(item.id) } />
                        <EditButton title="Edit" onClick={() => handleEdit(item.id) }/>
                      </Box>
                    </StyledTableCell>
                  </StyledTableRow>)
                  :
        ...
        {id ? edit ? <EditDetails handleClose={handleClose} id={id} /> : <View handleClose={handleClose} id={id} /> : null}
    </Box>
  )
}
